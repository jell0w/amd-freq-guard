...
#[tauri::command]
pub async fn save_trigger_action(
    app: AppHandle,
...
pub async fn load_trigger_actions(app: AppHandle) -> Result<Vec<TriggerAction>, String> {
    ...
    let content = fs::read_to_string(actions_path)
        .map_err(|e| format!("读取触发动作失败: {}", e))?;
    
    serde_json::from_str(&content)
        .map_err(|e| format!("解析触发动作失败: {}", e))
}
...
fn save_trigger_actions(app: &AppHandle, actions: &[TriggerAction]) -> Result<(), String> {
    ...
    let json = serde_json::to_string_pretty(actions)
        .map_err(|e| format!("序列化触发动作失败: {}", e))?;
    
    fs::write(actions_path, json)
        .map_err(|e| format!("保存触发动作失败: {}", e))
} 